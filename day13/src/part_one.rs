...
fn find_smallest_token(machine: &Machine) -> Option<usize> {
    ...
    let combos: Vec<[usize; 2]> = (0..=max_a_presses)
        .filter_map(|presses| {
            ...
            for m in 0..=max_b_presses {
                if current_x + (machine.b.x as usize * m) > machine.prize.x as usize {
                    ...
                    return None;
                } else if current_x + (machine.b.x as usize * m) == machine.prize.x as usize
                    && current_y + (machine.b.y as usize * m) == machine.prize.y as usize
                {
    ...
    combos
        .iter()
        .map(|presses| (presses[0] * 3 + presses[1]))
        .min()
}
