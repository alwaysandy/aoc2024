...
pub fn solve_part_two(input: &[Robot]) {
    ...
    let largest_quadrant = (0..100000).fold([0, 0], |acc, n| {
        ...
        let largest_quadrant = *count_quadrants(&robots, width, height).iter().max().unwrap();
...
fn count_quadrants(robots: &HashMap<XY, usize>, width: i32, height: i32) -> [i32; 4] {
    robots.iter().fold([0, 0, 0, 0], |acc, (robot, count)| {
        ...
        } else if robot.x >= width / 2 + 1 && robot.x <= width && robot.y >= 0 && robot.y <= height / 2 - 1 {
            [
                acc[0],
                acc[1] + (*count as i32),
                acc[2],
        ...
        } else if robot.x >= 0 && robot.x <= width /2 - 1 && robot.y >= robot.y / 2 + 1 && robot.y <= height {
            [
                acc[0],
                acc[1],
                acc[2] + *count as i32,
        ...
        } else if robot.x >= width / 2 + 1 && robot.x <= width && robot.y >= height / 2 + 1 && robot .y <= height {
            [
                acc[0],
                acc[1],
                acc[2],
